...
public class JdbcTransactionTemplate {
    ...
    public <T> T execute(Supplier<T> action, String jdbcUrl) {
    ...
    public void execute(Consumer<Connection> consumer) {
        execute(consumer, Connection.TRANSACTION_REPEATABLE_READ);
    }

    public void execute(Consumer<Connection> consumer, int isolationLvl) {
        Connection connection = null;
        try {
            connection = holder.connection();
            connection.setTransactionIsolation(isolationLvl);
